"""HTTP API АБУ: оркестрация в ДВБ, псевдо-ИИ и numpy — в недоверенной зоне."""
...
def tick_step() -> dict[str, Any]:
    """Один шаг симуляции: сенсоры, недоверенные подсказки, проверки ДВБ."""
    global _mission
    if _mission is None:
    ...
    smooth_vib = smooth_vibration_window(m.vibration_samples)
    default_log.record(
        EventLevel.INFO,
        f"tick depth={m.depth_m} smooth_vib={smooth_vib:.4f}",
    )
    ...
    if should_emergency_stop(risk, vib_score):
        m.status = "emergency"
...
def ai_suggest(body: AISuggestIn) -> dict[str, float]:
    """Псевдо-ИИ: рекомендации режима (делегирование в other)."""
